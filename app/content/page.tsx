  ...
  return (
    <div className="min-h-screen bg-white text-black pt-24">
      <main className="container mx-auto max-w-5xl px-4 py-16">
        ...
        <p className="text-lg text-gray-700 mb-6 text-center font-sans">Insights, tutorials, and resources from the forefront of applied AI.</p>

        {contentItems.length === 0 ? (
          <p className="text-center text-gray-700 font-sans">Could not load content items.</p>
        ) : (
              ...
              >
                ...
                >
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    <span className="font-mono text-[10px] tracking-widest text-gray-700 mb-2 uppercase">{item.tag}</span>
                    <h2 className="font-heading text-lg mb-2 text-black group-hover:text-gray-800 transition-colors">{item.title}</h2>
                    ...
                    <span className="mt-auto font-mono text-[11px] tracking-wider text-black group-hover:underline">
                      {
                        item.tag === 'community' ? 'Join →' :
                        (item.tag === 'newsletter' || item.tag === 'article') ? 'Read →' :
                        (item.tag === 'workshop' || item.tag === 'video') ? 'Watch →' :
                        'View Content →' // Default fallback
                      }
                    </span>
                  </div>
                </Link>
